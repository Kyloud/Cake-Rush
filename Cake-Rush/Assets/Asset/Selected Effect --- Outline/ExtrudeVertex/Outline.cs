...
namespace SelectedEffectOutline
{
	...
	public class Outline : MonoBehaviour
	{
		...
		void Update()
		{
			...
			// calculate dash outline center
			Vector2 scruv = new Vector2(0.5f, 0.5f);
			if (m_Camera)
			{
				Vector3 scrpos = m_Camera.WorldToScreenPoint(m_Trans.position);
				scruv = new Vector2(scrpos.x / Screen.width, scrpos.y / Screen.height);
			}
			...
			for (int i = 0; i < mats.Length; i++)
			{
				if (m_Dash)
				{
					...
					//mats[i].SetVector("_OutlineDashCenter", new Vector4(scruv.x, scruv.y, 0f, 0f));
				}
				else
				{
