	...
	SubShader {
		...
		Pass
		{
			...
			half4 frag (v2f input) : SV_Target
			{
				...
				half up = tex2D(_MainTex,    uv + fixed2(0, _MainTex_TexelSize.y * _OutlineThickness)).a;
				half down = tex2D(_MainTex,  uv - fixed2(0, _MainTex_TexelSize.y * _OutlineThickness)).a;
				half right = tex2D(_MainTex, uv + fixed2(_MainTex_TexelSize.x * _OutlineThickness, 0)).a;
				half left = tex2D(_MainTex,  uv - fixed2(_MainTex_TexelSize.x * _OutlineThickness, 0)).a;
#if OUTLINE_ONLY
				c = 0.0;
#endif
